...
static const size_t kAlignedSharedUniformsSize =
(sizeof(SharedUniforms) & ~0xFF) + 0x100;
...
(id<RenderDestinationProvider>)renderDestinationProvider {
    self = [super init];
    if (self) {
        _session = session;
        _device = device;
...
- (void)update {
    ...
    _renderDestination.currentRenderPassDescriptor;
...
- (void)_loadMetal {
    ...
    const NSUInteger sharedUniformBufferSize =
    kAlignedSharedUniformsSize * kMaxBuffersInFlight;
    const NSUInteger anchorUniformBufferSize =
    kAlignedInstanceUniformsSize * kMaxBuffersInFlight;
    
    // Create and allocate our uniform buffer objects. Indicate shared storage so
    // that both the
    //   CPU can access the buffer
    _sharedUniformBuffer =
    [_device newBufferWithLength:sharedUniformBufferSize
                         options:MTLResourceStorageModeShared];
    
    _sharedUniformBuffer.label = @"SharedUniformBuffer";
    
    _anchorUniformBuffer =
    [_device newBufferWithLength:anchorUniformBufferSize
                         options:MTLResourceStorageModeShared];
    
    _anchorUniformBuffer.label = @"AnchorUniformBuffer";
    
    // Create a vertex buffer with our image plane vertex data.
    _imagePlaneVertexBuffer =
    ...
    MTLVertexDescriptor *imagePlaneVertexDescriptor =
    [[MTLVertexDescriptor alloc] init];
    
    // Positions.
    imagePlaneVertexDescriptor.attributes[kVertexAttributePosition].format =
    ...
    // Texture coordinates.
    imagePlaneVertexDescriptor.attributes[kVertexAttributeTexcoord].format =
    MTLVertexFormatFloat2;
    ...
    capturedImagePipelineStateDescriptor.sampleCount =
    _renderDestination.sampleCount;
    capturedImagePipelineStateDescriptor.vertexFunction =
    capturedImageVertexFunction;
    capturedImagePipelineStateDescriptor.fragmentFunction =
    capturedImageFragmentFunction;
    capturedImagePipelineStateDescriptor.vertexDescriptor =
    imagePlaneVertexDescriptor;
    ...
    NSError *error = nil;
    _capturedImagePipelineState = [_device
                                   newRenderPipelineStateWithDescriptor:capturedImagePipelineStateDescriptor
                                   error:&error];
    ...
    capturedImageDepthStateDescriptor.depthCompareFunction =
    MTLCompareFunctionAlways;
    capturedImageDepthStateDescriptor.depthWriteEnabled = NO;
    _capturedImageDepthState = [_device
                                newDepthStencilStateWithDescriptor:capturedImageDepthStateDescriptor];
    
    // Create captured image texture cache
    CVMetalTextureCacheCreate(NULL, NULL, _device, NULL,
    ...
    // Positions.
    _geometryVertexDescriptor.attributes[kVertexAttributePosition].format =
    ...
    // Texture coordinates.
    _geometryVertexDescriptor.attributes[kVertexAttributeTexcoord].format =
    MTLVertexFormatFloat2;
    ...
    // Normals.
    _geometryVertexDescriptor.attributes[kVertexAttributeNormal].format =
    ...
    anchorPipelineStateDescriptor.fragmentFunction =
    anchorGeometryFragmentFunction;
    ...
    _anchorPipelineState = [_device
                            newRenderPipelineStateWithDescriptor:anchorPipelineStateDescriptor
                            error:&error];
    ...
    _anchorDepthState =
    [_device newDepthStencilStateWithDescriptor:anchorDepthStateDescriptor];
...
- (void)_loadAssets {
    ...
    // attribute
    vertexDescriptor.attributes[kVertexAttributePosition].name =
    MDLVertexAttributePosition;
    vertexDescriptor.attributes[kVertexAttributeTexcoord].name =
    MDLVertexAttributeTextureCoordinate;
    vertexDescriptor.attributes[kVertexAttributeNormal].name =
    MDLVertexAttributeNormal;
    
    // Use ModelIO to create a box mesh as our object
    MDLMesh *mesh =
    [MDLMesh newBoxWithDimensions:(vector_float3){.075, .075, .075}
...
- (void)_updateBufferStates {
    ...
    _anchorUniformBufferOffset =
    kAlignedInstanceUniformsSize * _uniformBufferIndex;
...
- (void)_updateAnchorsWithFrame:(ARFrame *)frame {
    ...
    MIN(frame.anchors.count, kMaxAnchorInstanceCount);
    ...
    for (NSInteger index = 0; index < anchorInstanceCount; index++) {
        ...
        anchorUniforms->modelMatrix =
        matrix_multiply(anchor.transform, coordinateSpaceTransform);
    }
...
- (id<MTLTexture>)_createTextureFromPixelBuffer:(CVPixelBufferRef)pixelBuffer
    ...
    CVReturn status = CVMetalTextureCacheCreateTextureFromImage(
                                                                NULL, _capturedImageTextureCache, pixelBuffer, NULL, pixelFormat, width,
                                                                height, planeIndex, &texture);
...
- (void)_updateImagePlaneWithFrame:(ARFrame *)frame {
    ...
    // viewport
    CGAffineTransform displayToCameraTransform = CGAffineTransformInvert([frame
    ...
    for (NSInteger index = 0; index < 4; index++) {
        ...
        CGPoint textureCoord =
        CGPointMake(kImagePlaneVertexData[textureCoordIndex],
...
- (void)_drawCapturedImageWithCommandEncoder:
(id<MTLRenderCommandEncoder>)renderEncoder {
    ...
    [renderEncoder setFragmentTexture:_capturedImageTextureY
                              atIndex:kTextureIndexY];
    [renderEncoder setFragmentTexture:_capturedImageTextureCbCr
                              atIndex:kTextureIndexCbCr];
...
- (void)_drawAnchorGeometryWithCommandEncoder:
(id<MTLRenderCommandEncoder>)renderEncoder {
    ...
    // Set mesh's vertex buffers
    for (NSUInteger bufferIndex = 0; bufferIndex < _cubeMesh.vertexBuffers.count;
         bufferIndex++) {
