...
- (void)setWKWebViewScrollEnabled:(BOOL)enabled {
    ...
    // This code does not seem to be necessary and more over, it makes the webview not to receive touch events.
/*
    // There is one subview as of iOS 8.1 of class WKScrollView
    for (UIView* subview in self->wkWebView.subviews) {
...
//    ]];
    ...
    self->wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, URL_TEXTFIELD_HEIGHT, self.view.frame.size.width, self.view.frame.size.height - URL_TEXTFIELD_HEIGHT)];
    ...
    // Needed to show alerts. Check the WKUIDelegate protocol and the runJavaScriptAlertPanelWithMessage method in this file :(
    self->wkWebView.UIDelegate = self;
    [self.view addSubview: self->wkWebView];
    
    // Create the bidge to communicate the ObjectiveC code with the JavaScript code in the WKWebView
    // The bridge will log all the communications (useful during development). Comment it out for a cleaner console (production).
//    [WebViewJavascriptBridge enableLogging];
    self->bridge = [WebViewJavascriptBridge bridgeForWebView:self->wkWebView];
    // TODO - IMPORTANT (Iker Jamardo): There seems to be a bug in the WebViewJavascriptBridge and the decisionHandler of one of the delegat methods is being called multiple times and iOS does not seem to like it. In essence, and AFAIK, this is a correct behavior because different pages need to be loaded (the requested page, the request to inject the JavaScript bridge code, ...) and all require to specify an allow policy. In order to fix it with a hack (without modifying WebViewJavascriptBridge) it is mandatory to listen to the webview delegate and handle the call of the decisionHandler here. Check the code of the decidePolicyForNavigationAction method to further understand the fix.
    [self->bridge setWebViewDelegate:self];
    // TODO: Change the inline functions for methods in this class to make the code clearer/cleaner.
    [self->bridge registerHandler:@"getPose" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        data = [NSString stringWithFormat:@"{\"position\":[%f,%f,%f],\"orientation\":[%f,%f,%f,%f]}", position[0], position[1], position[2], orientation[0], orientation[1], orientation[2], orientation[3]];
        responseCallback(data);
    }];
    [self->bridge registerHandler:@"getProjectionMatrix" handler:^(id data, WVJBResponseCallback responseCallback) {
        ARFrame *currentFrame = [self.session currentFrame];
        matrix_float4x4 m4x4 = [currentFrame.camera projectionMatrixWithViewportSize:self.renderer->viewportSize orientation:UIInterfaceOrientationLandscapeRight zNear:0.001 zFar:1000];
//        matrix_float4x4 m4x4 = currentFrame.camera.projectionMatrix;
        const float* m = (const float*)(&m4x4);
        data = [NSString stringWithFormat:@"[%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f]", m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9], m[10], m[11], m[12], m[13], m[14], m[15]];
        responseCallback(data);
    }];
    [self->bridge registerHandler:@"disableCameraRender" handler:^(id data, WVJBResponseCallback responseCallback) {
        self.renderer.cameraRenderEnabled = false;
        responseCallback(nil);
    }];
    [self->bridge registerHandler:@"enableCameraRender" handler:^(id data, WVJBResponseCallback responseCallback) {
        self.renderer.cameraRenderEnabled = true;
        responseCallback(nil);
    }];
    [self->bridge registerHandler:@"getPointCloud" handler:^(id data, WVJBResponseCallback responseCallback) {
        ARFrame *currentFrame = [self.session currentFrame];
        NSUInteger numberOfPoints = currentFrame.rawFeaturePoints.count;
        data = [NSString stringWithFormat:@"{\"numberOfPoints\":%ld,\"points\":[", numberOfPoints];
        for (NSInteger i = 0; i < numberOfPoints; i++)
        {
            vector_float3 point = currentFrame.rawFeaturePoints.points[i];
            data = [data stringByAppendingString:[NSString stringWithFormat:@"%f,%f,%f%@", point[0], point[1], point[2], (i < numberOfPoints - 1 ? @"," : @"")]];
        }
        data = [data stringByAppendingString:@"]}"];
        responseCallback(data);
    }];
    [self->bridge registerHandler:@"hitTest" handler:^(id data, WVJBResponseCallback responseCallback) {
...
//        NSArray<ARHitTestResult *> * hits = [currentFrame hitTest:point types:(ARHitTestResultType)ARHitTestResultTypeExistingPlane];
        if (hits.count > 0)
        {
            ...
            point[1] = matrix[13];
            point[2] = matrix[14];
            data = [NSString stringWithFormat:@"{\"p\":[%@],\"point\":[%f,%f,%f],\"plane\":[%f,%f,%f,%f]}", data, point[0], point[1], point[2], plane[0], plane[1], plane[2], plane[3]];
        }
        else
        {
            data = nil;
...
//        NSLog(@"WebARKit: hitTest hits count for (%f, %f) = %ld - %@", x, y, hits.count, data);
    ...
    self->urlTextField = [[UITextField alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, URL_TEXTFIELD_HEIGHT)];
...
- (void)session:(ARSession *)session didUpdateFrame:(ARFrame *)frame
{
    ...
    position[1] = matrix[13];
    position[2] = matrix[14];
    NSString* jsCode = [NSString stringWithFormat:@"if (window.WebARKitSetPose) window.WebARKitSetPose({\"position\":[%f,%f,%f],\"orientation\":[%f,%f,%f,%f]});", position[0], position[1], position[2], orientation[0], orientation[1], orientation[2], orientation[3]];
    [self->wkWebView evaluateJavaScript:jsCode completionHandler:^(id data, NSError *error) {
        if (error)
        {
            [self showAlertDialog:[NSString stringWithFormat:@"ERROR: Evaluating jscode to provide pose: %@", error] completionHandler:^{
            }];
        }
    }];
}
...
- (void)session:(ARSession *)session cameraDidChangeTrackingState:(ARCamera *)camera
{
    ...
    NSLog(@"AR camera tracking state = %@%@", trackingStateString, (trackingStateReasonString != nil ? trackingStateReasonString : @""));
    
    // Only the first time the tacking state is something else but unavailable load the initial page.
    if (camera.trackingState != ARTrackingStateNotAvailable && !self->initialPageLoadedWhenTrackingBegins)
    {
        // Retore a URL from a previous execution and load it.
        NSString* urlString = [self getURLFromUserDefaults];
        if (urlString)
        {
            // As the code bellow does not allow to store invalid URLs, we will assume that the URL is correct.
